...
void MyAnalysis::SlaveBegin(TTree * /*tree*/)
{
   ...
   for(int ich=0; ich < 2; ich++){
     for(int i=0; i < 21; i++){
      ...
      h_HMass_m[ich][i] = new TH1D(Form("h_HMassM_Ch%i_S%i_%s",ich,i,option.Data()), "HMass (medium)",75 ,50 ,200);
      ...
      h_HMass_t[ich][i] = new TH1D(Form("h_HMassT_Ch%i_S%i_%s",ich,i,option.Data()), "HMass (tight)", 75 ,50 ,200);
