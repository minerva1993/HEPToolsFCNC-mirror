...
void recoEff::Begin(TTree * /*tree*/)
{
   TString option = GetOption();
}

void recoEff::SlaveBegin(TTree * /*tree*/)
{
   TString option = GetOption();
...
Bool_t recoEff::Process(Long64_t entry)
{
...
*/
      for ( size_t i=0; i<4; ++i ) {
        ...
        jetP4sDR[i] = jetP4sDR[i] * jet_JER_Nom[j];
      }
    }
...
void recoEff::SlaveTerminate()
{
  TString option = GetOption();
}
   

void recoEff::Terminate()
{
  TString option = GetOption();
...
double recoEff::transverseMass( const TLorentzVector & lepton, const TLorentzVector & met){
