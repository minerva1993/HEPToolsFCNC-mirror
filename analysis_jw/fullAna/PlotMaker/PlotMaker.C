...
void PlotMaker(){
  ...
  m_Monte[ttbb] = new MonteFile("ttbb","ttbb", kRed+4, 365.4);
  m_Monte[ttbj] = new MonteFile("ttbj", "ttbj", kRed+3, 365.4);
  m_Monte[ttcc] = new MonteFile("ttcc", "ttcc", kRed+2, 365.4);
  //m_Monte[ttjj] = new MonteFile("ttjj", "ttjj", kRed+1, 365.4);
  m_Monte[ttLF] = new MonteFile("ttLF", "ttLF", kRed, 365.4);
  m_Monte[ttBkg] = new MonteFile("ttBkg", "ttBkg", kPink, 365.4);
  ...
  h_tmp->SetLineColor(kBlack);
    ...
    if(channel == 0) {
      ...
      string hName = *v_itr + "DataSingleEl";
    ...
    h_div->GetXaxis()->SetTitleSize(0.08);
