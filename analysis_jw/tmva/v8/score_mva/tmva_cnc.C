...
void tmva_cnc::Begin(TTree * /*tree*/)
{
...
void tmva_cnc::SlaveBegin(TTree * /*tree*/)
{
  ...
  h_scoreBDT = new TH1D(Form("h_scoreBDT_%s",option.Data()), "BDT score", 50, -0.3, 0.3);
  h_scoreBDT->SetXTitle("BDT score");
  h_scoreBDT->Sumw2();
  fOutput->Add(h_scoreBDT);

  h_scoreKeras = new TH1D(Form("h_scoreKeras_%s",option.Data()), "Keras score", 50, 0, 1.0);
  h_scoreKeras->SetXTitle("Keras score");
  h_scoreKeras->Sumw2();
  fOutput->Add(h_scoreKeras);
}

Bool_t tmva_cnc::Process(Long64_t entry)
{
...
void tmva_cnc::Terminate()
{
  ...
  TFile * out = TFile::Open(Form("cnc_%s.root",option.Data()),"RECREATE");
